// pages/inbound/detail/detail.js
...
Page({
  ...
  async _loadDetail() {
    try {
      const order = await inboundApi.getDetail(this.data.id)
  ...
  async onApprove() {
    try {
      await inboundApi.updateStatus(this.data.id, 2)
  ...
  async onReject() {
    wx.showModal({
      ...
      success: async (res) => {
        ...
        try {
          await inboundApi.updateStatus(this.data.id, 3)
